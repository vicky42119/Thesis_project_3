...
class Record():
    ...
    def on_stream_com(self, data):
        # callback for receiving performance metrics data
        performance_metrics = data['com'][0]['performanceMetrics']

        # Process the performance metrics (calculate average values)
        avg_performance_metrics = {}
        for metric, values in performance_metrics.items():
            avg_performance_metrics[metric] = sum(values) / len(values)

        print('Performance Metrics:', avg_performance_metrics)

        # Store metrics in data structure
        self.data_structure.append({'time': time.time(), 'data': avg_performance_metrics})
...
def main():
    ...
    your_app_client_id = '4qDnpr090X8VQR7VCFSTSf5UbiQmE3RM29wAyoRR'
    your_app_client_secret = 'K7slmicboRcFlpEEWgyUd2F0oV9NYl8N3kcNG28EPxECHAwhzUT2J7avrFp7yINw6Hj0ryK6nOhXz59qriW4LKrDtG4qLTOTemCUwN7cT26eS3lTL3jqDORqfdHZdurC'
    ...
    r.record_title = 'thesis' # required param and can not be empty
    r.record_description = '' # optional param

    # input params for export_record. Please see on_warn_cortex_stop_all_sub()
    r.record_export_folder = '~/Desktop' # your place to export, you should have write permission, example on desktop
